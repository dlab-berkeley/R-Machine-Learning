...
Train a model to predict an individual's income `total_indiv_income` on `jobs_train` and evaluate the model on `jobs_test`. Note that this is related to, but not a direct function of, one's wage because income can come from multiple sources (e.g., retirement savings, investment income).
