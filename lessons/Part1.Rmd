...
In Challenge 1, we looked at the relationship between a continuous x feature, `age`, and our target, `hourly_wage`. Perform your own exploratory analyses, but this time examine at least one categorical feature (or features) that you think might be useful in predicting `hourly_wage`. In the companion resource guide, we provide a comprehensive data dictionary. Hint: one way to explore continuous variables is looking at overlapping histograms. Which features seem most interesting to you?
...
What are the data types for these three variables? What are some issues that might arise if we were to plug these data into a linear regression model?
...
```{r}
chal4_recipe <- 
  recipe(total_indiv_income ~ ., data = jobs) %>% # tell r what our outcome variable is 
...
chal4_metrics(chal4_results, truth = total_indiv_income, estimate = .pred)
```
