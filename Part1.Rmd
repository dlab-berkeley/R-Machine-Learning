...
[kq: find graph that doesn't use IV and DV language]
...
To ground symbols in code, let's explore our primary data set for Part 1 of the workshop: the Current Population Survey (CPS).
...
🔔 **Question 1**: Notice that we have called *y* the "outcome" or "target" variable, and the set of $\mathbf{X}$ variables "features" or "inputs." How does this language reflect a conceptual shift from traditional statistics, where we typically use language like "independent" and "dependent" variables?
...
We will follow these broad steps to develop and refine our predictions: 

![Machine Learning Pipeline](../images/ML%20Workflow.png)


# Step 1: Pre-Processing

Sometimes, our features are not in the best format for our model to use effectively. Data are messy, and often need to be transformed for machine learning model to be able to be fit.
...
🔔 **Question 2**: What are the data types for these three variables? What are some issues that might arise if we were to plug these data into a linear regression model?

**Solution:** Looking at the first 10 rows, we see that `hours_weekly` is a numeric variable with missing values; `union` is a binary categorical variable with two values; and `state` is categorical variable with many values. If we were to include these variables in a linear regression model as is, we would run into a few issues.
...
wage_recipe <- 
  ...
  step_normalize(all_of(c("age", "hours_weekly")))
wage_recipe
```
...
-   The function `step_normalize()` normalizes/standardizes the variables so that they have a mean of zero and a standard deviation of one. This is often done for continuous variables before fitting a machine learning model to make sense of coefficients.
...
Thus far, we have created a recipe, or a set of instructions, but we have not applied it to data. To do so, we need to use two functions: `prep()` and `bake()`.
...
```{r}
baked_jobs <- bake(prepped_recipe, new_data = jobs)
baked_jobs 
```

🔔 **Question 2**: Take a look at the columns in the `baked_jobs` data. What has changed?
...
```{r fit_model}
fitted_model <- model %>% 
  fit(hourly_wage ~ age + race + education + sex + hispanic + marital_status + worker_status + union + hours_weekly + as.factor(state),
      data = jobs_train)
fitted_model
```
...
## 🥊 Challenge 4: Other models

There are a variety of models you can fit using `parsnip`: [here](https://parsnip.tidymodels.org/reference/index.html)). Try out different models on your own and analyze what you're seeing in the model output.
...
To use this function, we need predictions from the model. The `predict()` method can be used to obtain a tibble with the predictions from our model on new data. We can match these values with the corresponding observed outcome values.
...
We can do multiple metrics at once, though. Let's pick three common metrics for linear regression models - $R^2$, Root Mean Square Error (RMSE), and Mean Absolute Error (MAE) - and evaluate them at the same time using `yardstick`.
...
We saw above that we could examine the coefficients of the fitted model. We can view a tidy version of these results to better analyze them:
...
For example, the age coefficient tells us that hourly_wage increases by roughly 7 cents for every one-year increase in age.

## [KQ: CONSIDER ADDING AGE2]
